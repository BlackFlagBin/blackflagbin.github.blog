...
可以看到 **subscribeOn** 与 **observeOn** 不同的是二者线程切换的地方。 **subscribeOn** 是将 **source.subscribe(observer)** 整个部分切换到了相应的线程，而 **observeOn** 则是在 **ObserveOnObserver** 这个包装类中的 **onNext** 、 **onError** 方法中做了线程的切换，具体是在 **onNext** 等方法中创建了相关的 **Worker** 工作

...
可以看到，Retrofit中的这个 **OKHttpCall** 完全是对OkHttp中的 **RealCall** 的一个包装。在 **OKHttpCall** 中的同步网络方法 **execute** 和异步网络方法 **enQueue** 中其实就做了三件事：获取 **RealCall** 对象，调用 **RealCall** 中的 **execute** 或者  **enQueue** 方法，然后去解析OkHttp返回的原始数据，就这么简单。
